...
`);a.gallery.innerHTML=i.join(" "),d.refresh()}function f(){a.gallery.innerHTML=""}function y(){a.loader.style.visibility="visible"}function g(){a.loader.style.visibility="hidden"}function h(){d=new m(".gallery a",{captionsData:"alt",captionPosition:"bottom",captionDelay:250})}a.form.addEventListener("submit",b);h();function b(s){if(s.preventDefault(),s.currentTarget.elements.search_text.value.trim().length===0){n.error({position:"topRight",message:"Search text is empty. Enter the text, please"});return}let i=s.currentTarget.elements.search_text.value.trim().split(" ").join("+");f(),y(),u(i).then(t=>{t.data.total?p(t.data.hits):n.error({position:"topRight",message:"Sorry, there are no images matching your search query. Please try again!"}),g()})}
